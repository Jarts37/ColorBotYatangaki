...
function NhgbCKYF(images){
//

  function fuhssyr(array) {
  ...
  var Cnvagre = function(config){
    ...
    function vfFnzrCvkryPbybe(coords){
    ...
    function tryToDraw(){
      for(var _y = 0; _y < canvas.height; _y++){
        for(var _x = 0; _x < canvas.width; _x++){
          ...
          if(vfFnzrCvkryPbybe(coords)){

          }
          else{

            var color_id = trgPbybeVq(coords);
  ...
  function draw(){
    ...
    if(0<timer){
      ...
      for(var i = 0; i < Cnvagres.length; i++){
        if(Cnvagres[i].isReady()){
          var result = Cnvagres[i].drawImage();
